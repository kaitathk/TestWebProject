...
(function(){
	...
	b=(function(){
		...
		c.prototype.set_default_values=function(){
			...
			this.enable_select_all=(this.options.enable_select_all!=null)&&this.is_multiple?this.options.enable_select_all:false;this.choices=0;
		...
		c.prototype.input_focus=function(d){var e=this;
			if(!this.active_field){return setTimeout((function(){return e.container_mousedown();}),50);}};
		c.prototype.input_blur=function(d){
			var e=this;
			if(!this.mouse_on_container){
				this.active_field=false;
				return setTimeout((function(){return e.blur_test();}),100);
			}};
...
(function(){
	...
	c=function(j,h){
		for(var g in h){
			if(b.call(h,g)){j[g]=h[g];}}
		function i(){this.constructor=j;}
		i.prototype=h.prototype;j.prototype=new i;
	...
	f=(function(g){c(h,g);
		...
		h.prototype.set_up_html=function(){
			...
			l=e("<div />",{id:this.container_id,"class":"chzn-container"+(this.is_rtl?" chzn-rtl":""),style:"width: "+this.f_width+"px;"});
			if(this.is_multiple){l.html('<ul class="chzn-choices"><li class="search-field"><input type="text" value="'+this.default_text+'" class="default" autocomplete="off" style="width:25px;" /></li></ul><div class="chzn-drop" style="left:-9000px;"><ul class="chzn-results"></ul></div>');}
			else{l.html('<a href="javascript:void(0)" class="chzn-single"><span>'+this.default_text+'</span><div><b></b></div></a><div class="chzn-drop" style="left:-9000px;"><div class="chzn-search"><input type="text" autocomplete="off" /></div><ul class="chzn-results"></ul></div>');}
			...
			this.dropdown.css({width:j+"px",top:k+"px"});
			...
			if(this.is_multiple){
				...
				this.search_field.css({width:i+"px"});
				}
			this.results_build();
			this.set_tab_index();
		...
		h.prototype.register_observers=function(){
			...
			if(this.is_multiple){
				this.search_choices.click(function(j){return i.choices_click(j);});
				return this.search_field.focus(function(j){return i.input_focus(j);});}
			else{
				return this.container.click(function(j){return j.preventDefault();});}
			};
		...
		h.prototype.container_mousedown=function(i){
			...
			if(!this.is_disabled){
				...
				if(i&&i.type==="mousedown"){i.stopPropagation();}
		...
		h.prototype.close_field=function(){
			...
			if(!this.is_multiple){this.selected_item.attr("tabindex",this.search_field.attr("tabindex"));
		...
		h.prototype.test_active_click=function(i){
			if(e(i.target).parents("#"+this.container_id).length){return this.active_field=true;}
		...
		h.prototype.results_build=function(){
			...
			j="";
			k=this.results_data;
			for(l=0,i=k.length;l<i;l++){m=k[l];
		...
		h.prototype.result_do_highlight=function(j){
			...
			if(j.length){
				...
				if(n>=l){return this.search_results.scrollTop((n-k)>0?n-k:0);}
					else{if(m<i){return this.search_results.scrollTop(m);}
			}}};
		...
		h.prototype.select_all_setup=function(){
			var i,j=this;
			i=e("<a />",{"class":"chzn-select-all"}).html(this.Select_all_text_default);
		...
		h.prototype.select_all_toggle=function(){
			...
			else{return this.select_all_enable()}};
		h.prototype.set_tab_index=function(j){
			...
			if(this.form_field_jq.attr("tabindex")){
				...
				else{this.selected_item.attr("tabindex",i);
		...
		h.prototype.result_select=function(j){
			...
			if(this.result_highlight){
				...
				if(!(j.metaKey&&this.is_multiple)){this.results_hide();}
				this.search_field.val("");
				this.form_field_jq.trigger("change");
				return this.search_field_scale()}};
						...
						if(!(this.is_multiple&&q.selected)){
							...
							if(u.test(q.html)){
								...
								if(q.html.indexOf(" ")>=0||q.html.indexOf("[")===0){
									...
									if(n.length){
										for(l=0,i=n.length;l<i;l++){k=n[l];
										if(u.test(k)){x=true;p+=1}}}}}
							if(x){
								if(w.length){
									r=q.html.search(j);
								...
								else{if(this.result_highlight&&s===this.result_highlight.attr("id")){
									this.result_clear_highlight()}
		...
		h.prototype.winnow_results_clear=function(){
			var i,l,m,k,j;
			this.search_field.val("");
			l=this.search_results.find("li");j=[];
		...
		h.prototype.no_results=function(i){
			var j;j=e('<li class="no-results">'+this.results_none_found+' "<span></span>"</li>');
		...
		h.prototype.search_field_scale=function(){
			...
			if(this.is_multiple){
				...
				n=i.width()+25;
				i.remove();
				if(n>this.f_width-10){n=this.f_width-10}
...
d=function(g){
	...
	a.get_side_border_padding=d}).call(this);